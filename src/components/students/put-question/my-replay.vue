    ...
    <div class="p-2 divide-y">
      <div class="mb-3 flex flex-col gap-3" v-for="(item, index) in queList" :key="index">
        ...
        <div class="text-sm text-gray-600 flex justify-between items-center" @click="viewDetail(item)">
          ...
          <div class="flex-1">
            状态：
            <span @click="goReplay(item)" v-if="item.status === '1'" class="color-o">已回复</span>
            <span v-if="item.status === '2'">未回复 </span>
            <span @click="goReplay(item)" v-if="item.status === '2'" class="color-g"> 去回复</span>
          </div>
          <div>
            <van-icon name="arrow" />
          </div>
...
const goReplay = (row) => {
  router.push({
    path: '/student-question-replay', query: {
