<template>
  <div class="w-full h-full">
    <van-nav-bar title="回复问题" left-text="返回" left-arrow @click-left="onClickLeft" />
    <div class="p-4">
      <p class="my-2">
      ...
      <p class="mt-2">回复状态：<span class="color-o text-sm">{{ routeInfo.status === '1' ? "已回复" : "未回复" }}</span></p>
      ...
      <p class="my-2" v-if="routeInfo.status === '2'">回复内容：<span class="text-sm text-gray-700">{{ routeInfo.date_re
      }}</span></p>
      <van-cell-group class="ring-1 ring-gray-500" inset>
        <van-field v-model="message" rows="4" autosize type="textarea" maxlength="250" placeholder="请输入回复内容"
          show-word-limit />
      </van-cell-group>
      <div class="mt-4">
        <van-button color="linear-gradient(to right, #ff6034, #ee0a24)" type="primary" size="large">提交</van-button>
      </div>
    </div>
  </div>
</template>
